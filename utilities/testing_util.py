...
def run_test(problem=None, problem_list:List[str]=None, prob_index:int=None, 
        eval:bool=False, test:str=None, debug:bool=False):
    """
    ...
    if not eval:
        ...
        if not problem.get("test_cases"):
            raise ValueError(
                "Evaluation test cases not found in the problem. Run with `eval=False`."
            )
        in_outs = problem["test_cases"]
        
    original_inputs = copy.deepcopy(in_outs["inputs"])
    expected_outputs = copy.deepcopy(in_outs["outputs"])
